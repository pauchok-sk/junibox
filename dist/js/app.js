 ...
 */
    ...
    var inputmask_min = __webpack_require__(604);
    ...
    function slider() {
        ...
        if (recSlider) {
            recSlider.querySelectorAll(".swiper-slide");
    ...
    function dropCatalog() {
        ...
        if (catalog) {
            catalogBtn.addEventListener("click", (e => {
                e.stopPropagation();
                if (catalog.classList.contains("_open")) closeCatalog(); else openCatalog();
            }));
    ...
    function fancy() {
        ...
        Oe.bind('[data-fancybox="video"]', {});
        Oe.bind('[data-fancybox="volumes"]', {});
    }
    ...
    function more() {
        ...
        if (moreParents.length) moreParents.forEach((p => {
            ...
            const minMax = parentItems.dataset.moreMinMax ? parentItems.dataset.moreMinMax : "max";
            const condition = media ? window.matchMedia(`(${minMax}-width: ${media}px)`).matches : 1;
            if (condition) {
    ...
    function specificationsAll() {
        ...
        if (specifications) btnSpecifications.addEventListener("click", (() => {
            specifications.classList.remove("_hidden");
            btnSpecifications.remove();
        }));
    }
    function sectionCard() {
        document.querySelectorAll(".section-card__search-btn");
        const sections = document.querySelectorAll(".section-card");
        if (sections.length && window.matchMedia("(max-width:479px)").matches) sections.forEach((section => {
            ...
            const maxHeight = currentBody.clientHeight > 480 ? currentBody.clientHeight : 480;
    ...
    function ranges() {
        ...
        if (ranges.length) ranges.forEach((range => {
            const currentSlider = range.closest(".selection-simple__wrapper-range").querySelector(".selection-simple__slider");
            const radios = range.closest(".selection-simple__wrapper-range").querySelectorAll(".selection-simple__input-radio");
            const checkedRadioIndex = Array.from(radios).findIndex((item => item.checked === true)) + 1;
            ...
            range.addEventListener("change", (e => {
                ...
                } else if (radios.length === 3) if (value <= 33) {
                    radios[0].checked = true;
                    offsetSliderChange(offset);
                } else if (value <= 66) {
                    radios[1].checked = true;
